...
class RateLimitedExecutionQueue(AbstractContextManager):
  def __init__(self, max_calls_per_interval: int, interval: int = 1):
  ...
  @staticmethod
  def _function_wrapper(limiter: BoundedSemaphore, function: Callable):
    limiter.acquire()
    return function()

  def add_function_to_pool(self, function: Callable):
